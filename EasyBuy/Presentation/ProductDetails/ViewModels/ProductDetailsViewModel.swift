...
class ProductViewModel: ObservableObject {
    ...
    func getDraftOrder(){
        ...
        FireBaseManager.shared.retriveCustomerDiscountCodes(id: customerID ?? "")?.getDocument(completion: { [weak self] snapshot, error in
            ...
            if self?.cart == nil{
                ...
                NetworkManager.getInstance(requestType: .admin).queryGraphQLRequest(query: DraftOrderQuery(id: (objFireBase?.draftOrders?.cartDraftOrder?.draftOrderCreate?.draftOrder?.id)!), responseModel: DraftOrderDataClass.self) {[weak self] res in
                    switch res {
                    case .success(let success):
                        print("draft order from shopify : \(success)")
                        var lineItems = self?.mapLineItemsToDratOrderLineItems(lineItems: success.draftOrderCreate?.draftOrder?.lineItems?.nodes ?? [])
                        lineItems?.append(DraftOrderLineItemInput(quantity: 1,variantId:  self?.product?.product?.variants?.edges?.first?.node?.id))
                        
                        self?.UpdateDraftOrder(id: objFireBase?.draftOrders?.cartDraftOrder?.draftOrderCreate?.draftOrder?.id ?? "", draftOrderInput: DraftOrderInput(lineItems: lineItems,purchasingEntity: PurchasingEntityInput(customerId: customerID)), customerDiscountCodes: objFireBase ?? CustomerDiscountCodes())
    ...
    func createDraftOrder(discountCodes: [String]){
        ...
        let linesItems = DraftOrderInput(lineItems:
                                            [DraftOrderLineItemInput(quantity: 1,variantId: product?.product?.variants?.edges?.first?.node?.id)] ,purchasingEntity: PurchasingEntityInput(customerId: customerID))
    ...
    func UpdateDraftOrder(id: String,draftOrderInput: DraftOrderInput,customerDiscountCodes: CustomerDiscountCodes){
        NetworkManager.getInstance(requestType: .admin).performGraphQLRequest(mutation: DraftOrderUpdateMutation(id: id, input: draftOrderInput), responseModel: DraftOrderDataClass.self) { res in
            switch res {
            case .success(let success):
                FireBaseManager.shared.saveCustomerDiscountCodes(customerDiscountCodes: CustomerDiscountCodes(id: customerDiscountCodes.id,discountCodes: customerDiscountCodes.discountCodes,draftOrders: DraftOrders(cartDraftOrder: success)))
            case .failure(let failure):
                print(failure)
            }
       }
